...
map_reads() {
 ...
 minimap2 -ax map-ont -N 3 -p 0.95 "$REF_INDEX" $READS \
   | samtools sort -@ 4 -o "$OUT_BAM"

  samtools index "$OUT_BAM"
  samtools idxstats "$OUT_BAM" > ${OUT_DIR}/${barcode}_${virus}.idxstats.tsv
  samtools view "$OUT_BAM" | cut -f1,3 > "${OUT_DIR}/${barcode}_${virus}.read_to_ref.tsv"
...
barcode_groups["barcode42"]="all_parvoviridae"
