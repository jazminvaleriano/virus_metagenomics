...
# ───── Input files ─────
bam_files <- c(
  "01_minimap/barcode42_all_parvoviridae.sorted.bam"
)
sample_names <- c("Barcode 42")
gff_file <- "genomes/parvovirus1.gff3"
contig <- "PP993478.1"  # Your single accession name

# ───── Load GFF and extract CDS annotations ─────
gff <- import(gff_file)
gff_df <- as.data.frame(gff)
gff_df <- gff_df %>%
  filter(seqnames == contig, type == "CDS", !is.na(product)) %>%
  group_by(product) %>%
  summarise(
...
# ───── Assign non-overlapping tracks (for stackingproduct# ───── Assign non-overlapping tracks (for stacking) ─────
...
gff_df$label <- gsub(" protein$", "", gff_df$product)
...
annotation_plot <- ggplot() +
  geom_rect(data = gff_df,
            aes(xmin = start, xmax = end, ymin = ymin, ymax = ymax, fill = product),
